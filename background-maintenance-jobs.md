...
以 `30 * * * * *`（每分钟第 30 秒）为例，`JOB_POLL_INTERVAL_MS = 30000`：
6 位 cron 格式为 `秒 分 时 日 月 周`，`30 * * * * *` 表示秒=30、其余通配，即每分钟的 XX:30 时刻触发。

```
tick 时刻       last_tick      cron.after(&last_tick)  take(1)   event <= now?   动作
───────────────────────────────────────────────────────────────────────────────────────
10:00:15        None           (跳过)                   -         -              设 last_tick=10:00:15
10:00:45        10:00:15       10:00:30,...             10:00:30  10:00:30 ≤ now  ✓ 执行
10:01:15        10:00:45       10:01:30,...             10:01:30  10:01:30 > now  break，不执行
10:01:45        10:01:15       10:01:30,...             10:01:30  10:01:30 ≤ now  ✓ 执行
```
...
以 `0 5 * * * *`（每小时第 5 分钟第 0 秒）为例：
`0 5 * * * *` 表示秒=0、分=5、其余通配，即每小时的 XX:05:00 触发。

```
tick 时刻       last_tick      cron.after(&last_tick)  take(1)   event <= now?   动作
───────────────────────────────────────────────────────────────────────────────────────
09:50:30        None           (跳过)                   -         -              last_tick=09:50:30
09:51:00        09:50:30       10:05:00,...             10:05:00  > now          break，不执行
 ...（中间 tick 均无匹配）...
10:05:00        10:04:30       10:05:00,...             10:05:00  10:05:00 ≤ now  ✓ 执行
10:05:30        10:05:00       11:05:00,...             11:05:00  > now          break
```

每小时 Job 的计划时刻之间间隔 60 分钟，`limit_missed_runs = 1` 意味着最多补跑 1 个小时。
由于清理类 Job 天然幂等，补跑 1 小时已足够。

#### 时序示例：Tick 延迟导致 Missed Run（每分钟 Job）

以 `30 * * * * *`（每分钟第 30 秒）为例，假设 tick 3 发生时系统卡顿，延迟到 10:05:15 才执行：

```
tick 时刻       last_tick      cron.after(&last_tick)              take(1)   event ≤ now?   动作
────────────────────────────────────────────────────────────────────────────────────────────────
10:00:15        None           (跳过)                               -         -              last_tick=10:00:15
10:00:45        10:00:15       10:00:30,...                         10:00:30  ≤ now          ✓ 执行 10:00:30
10:05:15        10:00:45       10:01:30,10:02:30,10:03:30,...      10:01:30  ≤ now          ✓ 执行 10:01:30
                                                                            ← take(1) 耗尽，循环结束
10:05:45        10:05:15       10:05:30,...                         10:05:30  ≤ now          ✓ 恢复正常
```

**10:02:30、10:03:30、10:04:30、10:05:30（即将到期但 last_tick 已跳过）四个计划时刻被永久丢失。**

原因：
- tick 3 在 10:05:15 时，`cron.after(&10:00:45)` 从 10:01:30 开始枚举
- `.take(1)` 只取 10:01:30 一个，执行后循环结束
- `last_tick` 被设为 10:05:15
- 下一个 tick 从 10:05:30 开始枚举，10:02:30~10:04:30 已落在 `last_tick` 之前，不会再被访问

#### 调度线程的阻塞场景

调度线程的主循环（[main.rs:757-760](src/main.rs#L757-L760)）：
...
`sched.tick()` 本身是纯同步调用：遍历所有 Job，检查 cron 匹配，对命中的 Job 调用闭包。闭包内部仅有 `runtime.spawn(...)` ——这是一个向 tokio 提交异步任务的**非阻塞操作**，不涉及 I/O，微秒级完成。因此 `sched.tick()` 本身不会成为阻塞源。

**实际可能造成 tick 延迟的场景**：

1. **OS 线程调度延迟**：`job-scheduler` 是普通优先级的 OS 线程。在 CPU 高负载下，OS 可能推迟调度该线程，导致 `sched.tick()` 被延迟调用。这不是代码层面的阻塞，而是系统级抢占。

2. **`runtime.block_on(sleep)` 的唤醒精度**：`tokio::time::sleep` 依赖 OS 定时器。在 Windows 上默认定时器分辨率为 15.6ms，实际唤醒可能在预期时刻 ±15ms 范围内。对于 `JOB_POLL_INTERVAL_MS = 30000`（30 秒）的量级，这可以忽略；但如果配置为很小的值（如 100ms），累积误差可能影响 cron 匹配精度。

3. **tokio Runtime 背压**：[main.rs:669](src/main.rs#L669) 为调度器创建了**独立的 tokio Runtime**（`let runtime = tokio::runtime::Runtime::new().unwrap()`）。所有 Job 的异步任务都提交到这个独立 Runtime。如果该 Runtime 的工作线程被大量并发任务占满（例如连接池耗尽导致多个任务同时等待），`runtime.spawn()` 仍然是非阻塞的（仅放入任务队列），不会延迟 `sched.tick()`。但 `runtime.block_on(sleep())` 需要等到 sleep future 被 Runtime 驱动完成——如果 Runtime 的事件循环严重积压，sleep 的唤醒可能被推迟。

4. **GC/内存压力**：Rust 本身无 GC，但数据库驱动或日志框架的大内存分配可能触发 OS 级的内存回收（如 swap），间接影响线程调度。

**不是阻塞源的场景**：

- **数据库慢查询**：Job 的异步任务在 tokio Runtime 中运行，与调度线程完全解耦。`pool.get()` 超时或慢查询只会影响 tokio 工作线程，不会阻塞 `sched.tick()` 主循环。
- **邮件发送延迟**：同上，SMTP 操作是异步的，在 tokio 任务中执行。
- **大量 Job 同时命中**：每个命中只是多一次 `runtime.spawn()` 调用，纳秒级操作。
